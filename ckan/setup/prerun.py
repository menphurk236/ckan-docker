...
def init_discovery():
    init_command = ['paster', '--plugin=ckanext-discovery', 'search_suggestions',
            'init', '-c' , ckan_ini]
    print('[prerun] Initializing discovery plugin - start')
    try:
        subprocess.check_output(init_command, stderr=subprocess.STDOUT)
        print('[prerun] Initializing discovery plugin - end')
    except subprocess.CalledProcessError as e:
        if 'OperationalError' in e.output:
            print(e.output)
            print('[prerun] Discovery plugin not ready, waiting a bit before exit...')
            time.sleep(5)
            sys.exit(1)
        else:
            print(e.output)
...
def opendstats_init():
    ...
    try:
        ...
        if 'OperationalError' in e.output:
            print(e.output)
            print('[prerun] Opendstats plugin not ready, waiting a bit before exit...')
            time.sleep(5)
            sys.exit(1)
        else:
            print(e.output)
